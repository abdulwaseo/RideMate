...
import { DriverRatePassengersModal } from '../../components/driver/DriverRatePassengersModal';
...
const ActiveRideInner: React.FC = () => {
  const { activeRide, requests, editRide, cancelRide, completeRide } = useDriver();
  ...
  const [showRatePassengersModal, setShowRatePassengersModal] = useState(false);
  const [ratePassengersData, setRatePassengersData] = useState<{
    rideId: string;
  ...
  const handleCompleteConfirm = async () => {
    setIsSubmitting(true);
    try {
      const rideIdToRate = activeRide?.id || '';
      const passengersToRate = requests
        .filter((r) => r.status === 'Accepted')
        .map((r) => ({ id: r.passengerId || r.id, name: r.passengerName }));

      stopDriverTracking();
      const success = await completeRide();
      if (success) {
        setShowCompleteModal(false);
        setRatePassengersData({
          rideId: rideIdToRate,
          passengers: passengersToRate,
        });
        setShowRatePassengersModal(true);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-6 space-y-6">
              <RideCard 
                ride={activeRide}
                actions={
                  <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2.5 w-full sm:w-auto select-none">
                    <Button
                      variant="glass"
                      size="sm"
                    ...
                    <Button
                      variant="primary"
                      size="sm"
                      className="w-full min-h-[44px] text-xs bg-brand-primary text-brand-bg hover:bg-brand-primaryLight font-bold"
                    ...
                    <Button
                      variant="danger"
                      size="sm"
                      className="w-full min-h-[44px] text-xs bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 font-bold"
            ...
            <div className="lg:col-span-6 space-y-4">
              <div className="bg-brand-card border border-brand-border rounded-2xl p-3.5 sm:p-4 shadow-glass space-y-3.5 sm:space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                  ...
                  <button
                    ...
                    className={`min-h-[44px] px-3.5 py-2 rounded-xl border text-xs font-bold transition-all shadow-md flex items-center justify-center gap-1.5 ${
                ...
                <MapContainer height="200px" className="sm:h-[280px]" />
                <TrackingStatus />
              </div>
      ...
      {/* Driver Rate Passengers Floating Modal - ALWAYS mounted */}
      <DriverRatePassengersModal
        isOpen={showRatePassengersModal}
        onClose={() => {
          setShowRatePassengersModal(false);
          navigate('/dashboard/driver');
        }}
        rideId={ratePassengersData.rideId}
        passengers={ratePassengersData.passengers}
        onSubmitted={() => {
          setShowRatePassengersModal(false);
          navigate('/dashboard/driver');
        }}
